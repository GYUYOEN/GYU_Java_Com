/*
 * 1. 관리자 계정으로 전환하여 다음의 권환을 가지는 사용자를 생성한다.
 *		A. deve01 계정을 생성하고 RESOURCE, CONNECT, CREATE SESSION 권한을 부여한다
 *		B. deve02 계정을 생성하고 CONNECT, CREATE SESSION, INSERT, UPDATE, DELETE 권한을 부여한다.
 *	    C. devAdmin 계정을 생성하고 RESOURCE, CONNECT, CREATE SESSION, INSERT, UPDATE, DELETE 권한을 부여한다
 *    위 권한을 부여 한 후 확인 까지 해본다.
 *
 * 2. devAdmin 계정으로 전환하여 다음의 작업을 수행한다.
 * 		A. 회원관리를 위한 테이블(USER_ACCOUNT)을 생성한다.
 * 		   식별값, 회원아이디, 회원패스워드, 회원명, 나이, 성별 정보를 관리할 수 있는 컬럼을 포함하도록 한다.
 * 		B. 회원관리에서 사용하는 식별값은 SUEQUENCE 객체를 이용하도록 한다.
...
CREATE TABLE USER_ACCOUNT(
	 ...
	 , NAME			VARCHAR2(30) CONSTRAINT NN_USER_ACCOUNT_NAEM NOT NULL
...
-- 위에 생성한 ID를 그대로 사용해주기 위해서 CURRVAL 사용.
