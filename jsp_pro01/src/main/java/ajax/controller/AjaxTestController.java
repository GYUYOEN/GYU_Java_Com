...
//		out.print("}"); // 이 형식(json형식)에 맞춰 보내주면 success에서 data양식 받고 받아온 데이터를 이용하여 출력
		...
		jg.writeStartObject(); // 객체를 쓰기 위한 준비 "{"
		...
		*/
		...
		om.writeValue(out, datas); // out : PrintWriter out = response.getWriter();
		
		response.setContentType("application/json; charset=utf-8"); // 데이터 타입이 json 이므로 작성
	}
