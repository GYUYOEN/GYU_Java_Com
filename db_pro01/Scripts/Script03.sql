...
-- LOCATIONS 테이블에서 COUNTRY_ID 가 US인 데이터 조회할 때
...
-- EMPLOYESS 테이블에서 MANAGER_ID 가 100 이고 DEPARTMENT_ID 가 50 인 데이터 조회
SELECT * FROM EMPLOYEES
 WHERE MANAGER_ID = 100
   AND DEPARTMENT_ID = 50;

-- EMPLOYEES 테이블에서 COMMISSION_PCT 가 NULL 이 아니면서 SALARY 가 10000 이상인
-- 데이터를 조회할 때 COMMISSION_PCT 가 포함된 급여로 조회
...
-- 예) EMPLOYEE_ID 가 100 인 사람은 EMAIL 이 SKING JOB_ID 가 AD_PRES 이다. 이를 문제에서
