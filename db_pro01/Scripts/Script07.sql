...
SELECT DEPARTMENT_ID
	 ...
	 , COUNT(SALARY)
  FROM EMPLOYEES
-- DEPARTMENT_ID와 JOB_ID 둘다 같은 거끼리 그룹으로 묶음
 GROUP BY DEPARTMENT_ID, JOB_ID
 ORDER BY DEPARTMENT_ID;
  
-- 묶인 그룹으로 출력이 되어야 하기 때문에 그냥 HIRE_DATE 쓰면 오류남 : GROUP BY 사용해서
-- GROUP BY에 쓰인거 그대로 SELECT에 쓰여야함 : 그룹으로 묶였기 때문에 묶인거 그대로 쓰여져야함
SELECT EXTRACT(YEAR FROM HIRE_DATE) AS 고용년
 ...
 -- HAVING(GROUP BY에 대한 조건) = WHERE(조건절, 그룹으로 묶기전에 사용해서 걸러냄)
 -- 1 빼고 출력
 HAVING COUNT(SALARY) > 1;

-- FROM > WHERE > GROUP BY > HAVING > SELECT > ORDER BY
SELECT COMMISSION_PCT
	 -- 그룹함수들은 그룹으로 묶고 나서 사용가능
	 , MIN(SALARY) AS "최저 급여"
	 , MAX(SALARY) AS "최고 급여"
	 , AVG(SALARY) AS "평균 급여"
	 , SUM(SALARY) AS "급여 총합"
...
-- HAVING : GROUP BY 해서 나온 결과들을 가지고 조건절 작성
...
-- TRUNC : 절사
-- SELECT에 그룹에서 요구하는 항목만 나오게 작성
-- 이후 선택이 된 항목들을 GROUP BY를 통해 묶어줌
...
-- EXTRACT(YEAR FROM HIRE_DATE) 는 숫자이기 때문에 문자열로 바꿔주고 SUBSTR을 사용해야함
 ...
 * 		12000 이상 5그룹
 */
-- HAVING 절에는 주로 그룹함수 사용
-- 묶을 그룹을 SELECT 절에 분류한 후 그대로 GROUP BY에 적용
...
-- ROLLUP과 CUBE 위부분은 같으나 CUBE에 JOB_ID 집계가 추가됨
...
-- (1, 2, 3), (1, 2), (2, 3), (1, 3), (1), (2), (3) : 가능한 나올 수 있는 모든 묶음
SELECT DEPARTMENT_ID
	 , JOB_ID
	 , DECODE(NVL(COMMISSION_PCT, 0), 0, 'NO', 'YES')
	 , SUM(SALARY)
